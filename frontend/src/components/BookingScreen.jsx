...
import {
  initiateBooking,
  processBookingData,
  fetchFlightDetails, // Ensure this service method exists
} from "../services/bookingService";
...
const BookingScreen = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        const details = await fetchFlightDetails(flightId); // Fetch flight details
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
      ...
      {flightDetails && (
        <Paper className="w-full max-w-md bg-white p-8 shadow-md rounded mb-6">
          ...
          <Typography variant="body1" className="mb-1">
            <strong>From:</strong> {flightDetails.source}
          </Typography>
          <Typography variant="body1" className="mb-1">
